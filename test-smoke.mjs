...
let res = await workerFetch(new Request('https://demo.pages.dev/'), {}, {});
...
res = await workerFetch(new Request('https://demo.pages.dev/', { headers: { cookie } }), {}, {});
body = await res.text();
check('T4 homepage served', res.status === 200 && body.includes('gdi-drives-grid') && body.includes('GDI Test'), 'status=' + res.status);

// T5: folder page /0:/
res = await workerFetch(new Request('https://demo.pages.dev/0:/', { headers: { cookie } }), {}, {});
body = await res.text();
check('T5 folder page served', res.status === 200 && body.includes('window.current_drive_order = 0'), 'status=' + res.status);

// T6: /sw.js proxy
res = await workerFetch(new Request('https://demo.pages.dev/sw.js', { headers: { cookie } }), {}, {});
...
res = await workerFetch(new Request('https://demo.pages.dev/download.aspx?file=zzz&expiry=zzz', { headers: { cookie } }), {}, {});
body = await res.text();
...
res = await workerFetch(new Request('https://demo.pages.dev/some/random/path', { headers: { cookie } }), {}, {});
...
// T9: KV binding injection — login_database is "Local" so ENV unused; simulate env passing anyway
res = await workerFetch(new Request('https://demo.pages.dev/login', {
  method: 'POST',
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  body: 'username=admin&password=admin123'
}), { ENV: { get: async () => null, put: async () => {} }, DB: null }, {});
check('T9 env binding injection (KV/DB globals) no-crash', res.status === 200, 'status=' + res.status);

// T10: 404 page for a real file that Drive says doesn't exist (mocked empty listing)
res = await workerFetch(new Request('https://demo.pages.dev/0:/nope.txt', { headers: { cookie } }), {}, {});
